  ...
  const editUser = (row_id: any, row_name: any, index: number) => {
    ...
    if (!isEdit) {
      ...
      if (access === decoded.accessToken) {
        ...
        if (decodedAccess.id === userData.id && decodedAccess.name === userData.name) {
          console.log('sdfsdf');
          setIsEdit(false);
        } else {
          axios.put('/api/users/update', userData).then((data) => {
            ...
            if (info.success) {
              ...
              window.localStorage.setItem('refreshToken', info.data.refreshToken);
                ...
                (row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell style={{ width: 160 }} align="center">
                      <IconButton aria-label="delete" color="primary" onClick={() => editUser(row.id, row.name, index)}>
                        {isEdit ? <Check /> : <Edit />}
                      </IconButton>
                      &nbsp;&nbsp;
                      <IconButton
          ...
          >
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-description">
                If you want to delete this user, press AGREE. Or not, press DISAGREE.
              </DialogContentText>
            </DialogContent>
            <DialogActions>
